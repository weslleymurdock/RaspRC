...
public class NRFService : BackgroundService
{
    private readonly TimeSpan _period = TimeSpan.FromMilliseconds(50);
    private readonly ILogger<NRFService> _logger;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
            ...
            await timer.WaitForNextTickAsync(stoppingToken))
        {
            ...
            try
            {
                ...
                await receiverService.Update(Data);
                 
                Data = string.Empty;
                // Sample count increment
                _executionCount++;
                _logger.LogInformation($"Executed NRFService - Count: {_executionCount}");
            }
            catch (Exception ex)
            {
